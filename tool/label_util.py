...
class Dynamic_labeler:
    def __init__(
        ...
        if self.labeling_method == "slope":
            # low, high = min(normalized_coef_list), max(normalized_coef_list)
            ...
            raise Exception(
                "Sorry, only slope,quantile and DTW labeling_method are provided for now."
            )

    def DTW_clustering(self, data):
        fitting_data = to_time_series_dataset(data)
        km_stock = TimeSeriesKMeans(
            n_clusters=self.dynamic_num,
            metric="dtw",
            max_iter=100,
...
class Worker:
        ...
        if self.method == "slice_and_merge":
            ...
            for tic in self.tics:
                (
                    ...
                    y_pred_list,
                    norm_coef_list,
                ) = self.get_turning_points(
                    data_ori=self.data_dict[tic],
                    tic=tic,
                    length_constrain=self.max_length_expectation,
                )
    ...
    def label(self, work_dir=os.getcwd()):
        ...
        if self.method == "slice_and_merge":
            ...
            for tic in self.tics:
                ...
                label, data_seg, label_seg, index_seg = self.get_label(
                    self.data_dict[tic],
                    turning_points,
                    norm_coef_list,
                    tic,
            ...
            if self.TSNE:
                interpolated_pct_return_data_seg = np.array(
                    self.interpolation(self.all_data_seg)
                )
                try:
    ...
    def get_label(
        ...
        label_seg = []
        index_seg = []
        self.dynamic_flag = Dynamic_labeler(
        ...
        for i in range(len(turning_points) - 1):
            ...
            if turning_points[i + 1] - turning_points[i] > 2:
                data_seg.append(
                    data.iloc[turning_points[i] : turning_points[i + 1]].to_list()
                )
    ...
    def preprocess(self, data):
        ...
        for tic in self.tics:
            tic_data = data.loc[
                data[self.tic] == tic, [self.timestamp, self.tic, self.key_indicator]
            ]
            tic_data.sort_values(by=self.timestamp, ascending=True)
            tic_data = tic_data.assign(
                pct_return=tic_data[self.key_indicator].pct_change().fillna(0)
            )
            self.data_dict[tic] = tic_data.reset_index(drop=True)
        for tic in self.tics:
            self.adjcp_apply_filter(
                self.data_dict[tic], self.Wn_key_indicator, self.order
            )
            self.data_dict[tic] = self.data_dict[tic].assign(
                pct_return_filtered=self.data_dict[tic]["key_indicator_filtered"]
                .pct_change()
                .fillna(0)
            )
    ...
    def tic_DTW(self, work_dir):
        ...
        for tic in self.tics:
            try:
                ...
                data_by_tic.append(
                    self.data_dict[tic]
                    .loc[:, [self.key_indicator, "pct_return"]]
                    .values
                )
        ...
        self.TSNE_plot(
            tsne_results, tic_label, "_tic_cluster", folder_name=self.plot_path
        )

    def plot_indicator(self, data, name):
        fig, ax = plt.subplots(1, 1, figsize=(20, 10), constrained_layout=True)
        if isinstance(data[self.timestamp][0], str):
            date = data[self.timestamp].apply(
    ...
    def plot_filter(self, data, name, low=6, high=32, K=12):
        ...
        filtered_data = sm.tsa.filters.bkfilter(
            data[[self.key_indicator, "pct_return"]], low, high, K
        )
        if isinstance(data[self.timestamp][0], str):
            date = data[self.timestamp][K:-K].apply(
                lambda x: datetime.strptime(x, "%Y-%m-%d")
            )
        else:
    ...
    def adjcp_apply_filter(self, data, Wn_indicator, order):
        data["key_indicator_filtered"] = self.butter_lowpass_filter(
            data[self.key_indicator], Wn_indicator, order
        )
    ...
    def calculate_distance(
        self, seg1, seg2, iteration_count, labeling_method="default"
    ):
    ...
    def get_turning_points(self, data_ori, tic, length_constrain=0):
        ...
        if length_constrain != 0:
            for i in range(1, len(turning_points) - 1):
                if (
                    turning_points[i][0] - turning_points_new[-1][0]
                    >= self.min_length_limit
                ):
                    # no need to merge
        ...
        for i in range(len(turning_points) - 1):
            x_seg = np.asarray(
                [j for j in range(turning_points[i][0], turning_points[i + 1][0])]
            ).reshape(-1, 1)
            ...
            normalized_coef_list.append(
                100
                * adj_cp_model.coef_
                / data["key_indicator_filtered"].iloc[turning_points[i][0]]
            )
        ...
        if self.merging_threshold != -1:
            ...
            while change and merging_round < 20:
                ...
                if self.merging_dynamic_constraint != float("inf"):
                    ...
                    # calculate the label
                    label, data_seg, label_seg_raw, index_seg = self.get_label(
                        data=data,
                        turning_points=turning_points_temp_flat,
                        normalized_coef_list=normalized_coef_list,
                        tic=tic,
                ...
                for i in tqdm(range(len(turning_points) - 1)):
                    ...
                    if have_next_index == False:
                        break
                    if (
                        turning_points[next_index][0] - turning_points[i][0]
                        < self.max_length_expectation
                    ):
                        left_distance = float("inf")
                        right_distance = float("inf")
                        this_seg = (
                            data["key_indicator_filtered"]
                            .iloc[turning_points[i][0] : turning_points[next_index][0]]
                            .tolist()
                        )
                        if i > 0 and i < len(turning_points) - 1:
                            ...
                            if left_index is not None:
                                left_neighbor = (
                                    data["key_indicator_filtered"]
                                    .iloc[
                                        turning_points[left_index][0] : turning_points[
                                            i
                                        ][0]
                                    ]
                                    .tolist()
                                ...
                                left_distance = self.calculate_distance(
                                    left_neighbor, this_seg, merging_round
                                )
                        if i < len(turning_points) - 2:
                            ...
                            if next_index_2 is not None:
                                right_neighbor = (
                                    data["key_indicator_filtered"]
                                    .iloc[
                                        turning_points[next_index][0] : turning_points[
                                            next_index_2
                                        ][0]
                                    ]
                                    .tolist()
                                ...
                                right_distance = self.calculate_distance(
                                    this_seg, right_neighbor, merging_round
                                )
                            else:
                                right_neighbor = (
                                    data["key_indicator_filtered"]
                                    .iloc[turning_points[next_index][0] :]
                                    .tolist()
                                )
                                right_distance = self.calculate_distance(
                                    this_seg, right_neighbor, merging_round
                        ...
                        if self.merging_dynamic_constraint != float(
                            "inf"
                        ) and next_index < len(label_seg):
                            # check right
                            if right_distance != float(
                                "inf"
                            ) and self.merging_dynamic_constraint < abs(
                                label_seg[i] - label_seg[next_index]
                            ):
                            ...
                            # check left
                            if i > 0:
                                if left_distance != float(
                                    "inf"
                                ) and self.merging_dynamic_constraint < abs(
                                    label_seg[i] - label_seg[left_index]
                                ):
                        ...
                        if min(left_distance, right_distance) < self.merging_threshold:
                            ...
                            if left_distance < right_distance:
                                turning_points[left_index] = (
                                    turning_points[left_index] + turning_points[i]
                                )
                            else:
                                turning_points[next_index] = (
                                    turning_points[i] + turning_points[next_index]
                                )
                            change = True
                            turning_points[i] = []
                ...
                print(
                    f"Merge prohibit times by merging_dynamic_constraint: {merge_prohibit_times}"
                )
            ...
            print(
                f"merging_round in total: {merging_round}, number of segments: {len(turning_points)}"
            )
    ...
    def plot(self, tics, parameters, data_path, model_id):
        self.plot_path = os.path.join(
            os.path.dirname(os.path.realpath(data_path)), model_id
        )
        ...
        if self.method == "slice_and_merge":
            ...
            for tic in tics:
                ...
                paths.append(
                    self.plot_to_file(
                        self.data_dict[tic],
                        tic,
                        self.y_pred_dict[tic],
                        self.turning_points_dict[tic],
                        low,
                        high,
                        normalized_coef_list=self.norm_coef_list_dict[tic],
                        folder_name=self.plot_path,
                        plot_feather=self.key_indicator,
                    )
                )
            ...
            try:
                self.TSNE_plot(
                    self.tsne_results, self.all_label_seg, folder_name=self.plot_path
                )
            except:
    ...
    def plot_to_file(
        ...
        fig, axs = plt.subplots(
            sub_plot_num, 1, figsize=(50, 15 * sub_plot_num), constrained_layout=True
        )
        if sub_plot_num == 1:
            axs = [axs]
        ...
        for index, ax in enumerate(axs):
            ...
            for i in range(len(turning_points_seg) - 1):
                x_seg = np.asarray(
                    [j for j in range(turning_points_seg[i], turning_points_seg[i + 1])]
                ).reshape(-1, 1)
                coef = normalized_coef_list[i + counter]
                if (
                    self.labeling_method == "slope"
                    or self.labeling_method == "quantile"
                ):
                    coef = coef[0]
                elif self.labeling_method == "DTW":
                    coef = i + counter
                ...
                ax.plot(
                    x_seg,
                    data[plot_feather].iloc[
                        turning_points_seg[i] : turning_points_seg[i + 1]
                    ],
                    color=colors[flag],
                    label="market style " + str(flag),
                )
        ...
        # set the title
        plt.title(
            f"Dynamics_of_{tic}_linear_{self.labeling_method}_{plot_feather}",
            fontsize=20,
        )
    ...
    def linear_regession_timewindow(self, data_ori, tic, adjcp_timewindow):
        ...
        data = data_ori.iloc[
            : adjcp_timewindow
            * (data_ori["key_indicator_filtered"].size // adjcp_timewindow),
            :,
        ]
        adjcp_window_data = [
            data[["key_indicator_filtered"]][
                i * adjcp_timewindow : (i + 1) * adjcp_timewindow
            ]
            .to_numpy()
            .reshape(-1)
            for i in range(data["key_indicator_filtered"].size // adjcp_timewindow)
        ]
        coef_list = []
        fig, ax = plt.subplots(2, 1, figsize=(20, 10), constrained_layout=True)
        ...
        for i, data_seg in enumerate(adjcp_window_data):
            x_seg = np.asarray(
                [i * adjcp_timewindow + j for j in range(adjcp_timewindow)]
            ).reshape(-1, 1)
            adj_cp_model = LinearRegression().fit(x_seg, data_seg)
            y_pred = adj_cp_model.predict(x_seg)
            ax[1].plot(
                [i * adjcp_timewindow + j for j in range(adjcp_timewindow)], y_pred
            )
            coef_list.append(adj_cp_model.coef_)
        return coef_list
